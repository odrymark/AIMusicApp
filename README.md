...
Since the server does not have a GPU, the LLM runs on the CPU (4 cores, 8GB RAM), resulting in long waiting times for both AI functionalities.
Waiting time should be around 5 minutes for recommendation. Mood classification heavily depends on lyrics length and can be anywhere between 5 to 10 minutes.
...
Backend (.NET 9):
  ...
  - Handles file uploads for songs and cover images, storing them in Cloudflare R2.
  - Stores metadata (song titles, artists, playlist info) in the database.

Database (PostgreSQL):
  - Stores user data, playlist data, song metadata, and relationships between users and content.
  - Optimized with indexes for search queries and supports relational integrity for playlists and song ownership.

Cloudflare R2:
  - Stores all binary assets including song files and cover images.
  - Files are accessed via pre-signed URLs generated by the backend, keeping the bucket private.
